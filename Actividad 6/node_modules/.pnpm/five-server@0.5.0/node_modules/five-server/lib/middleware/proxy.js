...
const proxyMiddleware = (options, baseURL, injectBody, enableCache) => {
    ...
    const doRequest = async (req, res, next) => {
        ...
        const myReq = request(opts, (request) => {
            ...
            if (shouldInject) {
                const inject = new injectCode_1.Inject(['</head>', '</html>', '</body>'], (0, injectCode_1.code)(url, baseURL, injectBody), enableCache);
